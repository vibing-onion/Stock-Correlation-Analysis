...
def direct_suspension_strategy2(data, decision_log, bought, price, hold_time, stock, suspension):
    for i in range(len(data)):
        if (suspension > 0 and data.iloc[i,int(data.iloc[i,3] == True)] < price) and suspension < 10:
            ...
            continue
        suspension -= 1
        if not bought and data.iloc[i,:].loc["90% Converge"] == True:
            bought = True
        ...
        if bought:
            if data.iloc[i,stock] >= (price * 1.03):
                ...
                suspension = 15
                continue
            if data.iloc[i,:].loc["10% Converge"] == True:
                bought = False
                hold_time = 0
                decision_log.append(data.iloc[i,:])
                continue
            ...
            if hold_time > 30:
                if data.iloc[i,stock] > (price * 0.9) and data.iloc[i,stock] <= (price * 1):
                    ...
                    hold_time = 0
                    decision_log.append(data.iloc[i,:])
                elif data.iloc[i,:].loc["10% Converge"] == True:
                    bought = False
                    hold_time = 0
                    decision_log.append(data.iloc[i,:])
                else:
...
def decider(data):
    ...
    #decision_log = RealConv.ConvTrading(data, decision_log)
...
def trade(stockA, stockB, data):
    ...
    decision_log = []
    r, decision_log = RealConv.RealConvSimulate(RealConv.ConvTrading(data, decision_log))
    RoR = (r - 10000)/10000
...
def process(stockA, stockB, swap, start_, end_):
    ...
    #data_view = CRSI.Conv_RSI_Indicator(data_view, float(ref_set[4]), float(ref_set[6]), 14)
    
    decision_log, RoR = trade(stockA, stockB, data_view)
