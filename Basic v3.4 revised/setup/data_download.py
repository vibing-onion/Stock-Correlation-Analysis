...
def main():
    ...
    [p1, p2, p3, p4] = [mp.Process(target = download, args = (industry_us[i::4],tick_us[i::4])) for i in range(4)]
    
    jobs = [p1, p2, p3, p4]
    for job in jobs:
        job.start()

    result = [download(industry_us[i::4], tick_us[i::4]) for i in range(4)]
    ...
    with open ("./temp_file/industry_us.json", "w") as file:
        json.dump(industry_us,file)
